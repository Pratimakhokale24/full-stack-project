...
import { screenCandidate } from '../frontend/src/services/geminiService';
...
const CandidatesStep: React.FC<CandidatesStepProps> = ({ jobDetails, onScreened, onBack }) => {
  ...
  const handleScreen = async () => {
    ...
    const screeningPromises = files.map(file => 
      screenCandidate(jobDetails, file)
        .catch(e => {
            console.error(`Failed to process ${file.name}`, e);
            // Return a consistent error object if file reading or screening fails
            return {
    ...
    const screenedCandidates = await Promise.all(
      screeningPromises.map(p => p.then(result => {
          updateProgress();
          return result;
      }))
    );
    
    onScreened(screenedCandidates);
    setIsLoading(false);
  };
    ...
    <div className="flex flex-col gap-6">
      <div>
        ...
        <p className="text-slate-500 mt-1">
          Upload candidate resumes (.txt, .pdf) to see how they match up against the <span className="font-semibold text-indigo-500">{jobDetails.title}</span> role.
        </p>
      </div>
      ...
      >
        ...
        <input ref={fileInputRef} id="file-upload" name="file-upload" type="file" className="sr-only" multiple accept=".txt,.pdf" onChange={handleFileChange} />
      </div>
      ...
      <div className="flex justify-between items-center mt-4">
        ...
        <Button onClick={handleScreen} disabled={isLoading || files.length === 0}>
          {isLoading ? 'Screening...' : 'Screen Candidates & Continue'}
        </Button>
      </div>
    </div>
  );
